...
public class Map {
	...
	int nextErode = rand.nextInt(300) + 600;
	int items_removed = 0;
	Menu fert_menu;
	Menu oil_menu;
	...
	public void addRiverBanks() {
		...
		Position bank0 = new Position(.14, .67);
		Position bank1 = new Position(.33, .67);
		Position bank2 = new Position(.54, .67);
	...
	public void addStreams() {
		running_water = new ArrayList<River>();
	...
	public void update() {
		...
		if (items_removed >= 10) {
			...
			if(power_ups.get(i).ready){
				power_ups.get((i + 1) % 2).ready = true;
			}else{
				power_ups.get(i).ready = true;
			}
		}
		...
		nextErode--;
		if (nextErode < 0){
			nextErode = rand.nextInt(300) + 600;
			int i = 0;
			while(!river_banks.get(rand.nextInt(river_banks.size())).findErode() && i < 10){
				i++;
			}
		}
		for (int i = 0; i < river_banks.size(); i++) {
			river_banks.get(i).update();
			if (river_banks.get(i).removePoints){
				game.player.player_score.addScore(-50);
				river_banks.get(i).removePoints=false;
